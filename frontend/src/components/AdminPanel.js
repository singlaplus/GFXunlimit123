...
function AdminPanel({ initialDailyReportSettingsPage = false, initialDailyReportPreviewPage = false }) {
      ...
      <div style={{
        ...
        <ul style={{ margin: 0, paddingLeft: 18, color: isDarkMode ? '#dbeafe' : '#334155', lineHeight: 1.55, fontSize: 11 }}>
          {items.map((item, index) => {
            const itemName = item.name || item.label || 'Value';
            const currencyName = item.currency || item.label || 'value';
            const uniqueKey = `${title}-${itemName}-${currencyName}-${item.count ?? 0}-${index}`;
            return (
              <li key={uniqueKey} style={{ marginBottom: 2 }}>
                {itemName}: {formatBreakdownCount(item.count ?? item.value, item)}
              </li>
            );
          })}
        </ul>
  ...
  const saveDailyReportSettings = async () => {
    try {
      ...
      setDailyReportSettingsMessage(err.code === 'ECONNABORTED'
        ? 'Saving timed out. Check that the backend and PostgreSQL are running.'
        : (err.response?.data?.detail || err.response?.data?.error || 'Failed to save settings. Please try again.'));
  ...
  const buildCompactDailyReportHtml = (summary = {}) => {
    const entries = Object.entries(summary || {}).filter(([key, value]) => {
      if (key === 'generatedAt') return false;
    ...
    const lines = entries.map(([key, value]) => {
      const label = String(key).replace(/([A-Z])/g, ' $1').replace(/_/g, ' ').replace(/\s+/g, ' ').trim();
  ...
  const getActiveDailyReportSmtp = () => ({
    ...(dailyReportSettings?.reportSmtp || {}),
    ...(dailyReportSmtpDraft || {})
  });

  const getSavedDailyReportSmtp = () => ({
    ...(dailyReportSettings?.reportSmtp || {})
  });

  const openDailyReportSmtpForm = async () => {
    const loadedSettings = await loadDailyReportSettings();
    const nextDraft = { ...((loadedSettings || dailyReportSettings).reportSmtp || {}) };
    setDailyReportSmtpDraft(nextDraft);
  ...
  const saveDailyReportSmtpSettings = async () => {
    try {
      ...
      const token = getEffectiveAuthToken();
      const savedSmtp = getActiveDailyReportSmtp();
      const response = await axios.post(`${API_BASE_URL}/admin/email/daily-report-settings`, {
        ...dailyReportSettings,
        reportSmtp: savedSmtp
      }, {
        headers: token ? { Authorization: `Bearer ${token}` } : {}
      });
      const mergedReportSmtp = { ...(dailyReportSettings?.reportSmtp || {}), ...savedSmtp };
      setDailyReportSettings((previous) => ({ ...previous, reportSmtp: mergedReportSmtp }));
      setDailyReportSmtpDraft(mergedReportSmtp);
  ...
  const verifyDailyReportSmtpSettings = async () => {
    try {
      ...
      const token = getEffectiveAuthToken();
      const activeSmtp = getActiveDailyReportSmtp();
      await axios.post(`${API_BASE_URL}/admin/email/verify`, activeSmtp, {
        headers: token ? { Authorization: `Bearer ${token}` } : {}
      });
      setDailyReportSmtpDraft(activeSmtp);
  ...
  const sendDailyReportNow = async () => {
    ...
    try {
      ...
      const token = getEffectiveAuthToken();
      const previewResponse = await axios.get(`${API_BASE_URL}/admin/email/daily-report-preview`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {}
      });

      const previewSummary = previewResponse.data?.summary || {};
      const previewHtml = previewResponse.data?.html || buildCompactDailyReportHtml(previewSummary);
      const activeSmtp = getSavedDailyReportSmtp();

      await axios.post(`${API_BASE_URL}/admin/email/send-test`, {
        ...activeSmtp,
        to: recipient,
