...
module.exports = (router, pool, verifyAdminLocal) => {
  ...
  // First, try to add the daily_report_settings column if it doesn't exist
  pool.query(`
    ALTER TABLE users
    ADD COLUMN IF NOT EXISTS daily_report_settings JSONB DEFAULT NULL;
  `).catch(err => {
    console.log('Column check:', err?.message || 'OK');
  });

  // Create daily_report_schedules table if it doesn't exist
  pool.query(`
  ...
  `).catch(err => {
    console.log('Schedule table check:', err?.message || 'OK');
  });
